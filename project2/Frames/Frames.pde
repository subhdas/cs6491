...
void setup() {               // executed once at the begining 
  ...
  myFace = loadImage("data/selfie.JPG");  // load image from file pic.jpg in folder data *** replace that file with your pic of your own face
  textureMode(NORMAL);
...
void draw() {      // executed at each frame
  ...
  F1rF0 = F0.invertedOf(F1);    // F0^{-1} * F1
  E0rF0 = F0.invertedOf(E0);    // F0^{-1} * E0
  F=F(F0); E=F(E0);
  for(int i=0; i<k; i++) {
    F=F.of(F1rF0);              // F * F0^{-1} * F1
    if(showFrames) {penFill(blue,3); showArrows(F);}
    E=F.of(E0rF0);              // F * F0^{-1} * E0
